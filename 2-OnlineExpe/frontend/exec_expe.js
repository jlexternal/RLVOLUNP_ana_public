/* exec_expe
    Runs the function executing the experiment.

    Requirements: task_data - JSON object with 2 arrays (traj, idx_blocks; see ../server.js)
...
function exec_expe(task_data) {
  ...
  // localize task data brought in from server
  ...
  var preload = {
    type: 'preload',
    images: function() {
      //debug: need to counterbalance this order in the next version
...
//debug : use this variable to test things in the experiment without having to wait/uncomment/disable
/*
var test_trial = {
  type: 'html-keyboard-response',
  ...
  on_finish: function (data) {
    ...
    $.ajax({
      ...
      .done((successFlag) => { console.log('testing POST successFlag: '+ successFlag); })
      .fail((xhr,txt,err) => { console.log('FAIL testing POST'); console.log(err); });
  }
};
*/
  ...
  var welcome_block = {
    ...
    stimulus: function () {
      ...
      '<p>Thank you for participating in the retest of our study. </p>',
  ...
  if (!skip_intro) {
    ...
    timeline.push(spit_long_instructions('introduction')); // push introductory instructions
    timeline.push(spit_long_instructions('keypress'));     // push simple introductory keypress training
  }
  ...
  var post_example_trial = {
    ...
    individual_durations: () => {
      if (debugFlag) {
          ...
          return {durations: [1000,1000,1000,1000,500]};
      }
    }
  };
...
if (!skip_intro) {
  ...
  for (var isesh=0; isesh<n_sessions; isesh++) {
    ...
    var cor_locs  = Array.from({length: n_trials}, () => Math.round(Math.random())); // 0s or 1s

    /* Localize stimuli for the current session */
    let stimulis;
    if (cond_order[isesh] == 2) { // for VOL session
      // counterbalance the appearance of blue/orange as first correct choice
      if (subj_num % 2 == 1) {
        stimulis = ['img/orange.png','img/blue.png'];
      } else {
    ...
    for (var itrl=0; itrl<n_trials; itrl++){
      ...
      if (cond_order[isesh] == 2) { // VOL handling
        if (irnd_curr % 2 == 0) { // switch correct choice at switch point
      ...
      if (cond_order[isesh] != 2) { // for REF and UNP, need to cycle through shapes
        if (isesh < 3) { // 1st half
          if (cond_order[isesh]==1) { //REF
            // this part probably needs to get cleaned up to be more legible (but it is still functional)
        ...
        on_finish: function(data) {
          ...
          if (this.data.cond_expe != 'VOL') {
            ...
            if (stims[resp_side][4] == 'b') {
                resp_data.choice_symbol = 99; //blue
            } else {
  ...
  var end_experiment_stim = {
    ...
    stimulus: 'End of experiment.<br><br>Thank you for your participation!<br><br>'+
              'In case you are not automatically redirected, the completion code is: 171FB13A<br><br>'+
  ...
  jsPsych.init({
    ...
    on_finish: function() {
      ...
      window.location = "https://app.prolific.co/submissions/complete?cc=12345678"; //debug : The current code is for the Pilot (Prolific)
    }
  });
