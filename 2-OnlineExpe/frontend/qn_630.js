...
// questionnaire battery from Marion Rouault & Tricia Seow converted for jsPsych 6.3.0

// Jun Seok Lee - May 2021 <jlexternal@gmail.com>

// EMPTY ARRAY FOR TIMELINE OF QUESTIONNAIRES
var qns = [];

function make_qvar(qs,qscale) {
	var q_arr = [];
	qs.forEach(q => q_arr.push({
		prompt: q,
		options: qscale,
		required: true
	}));
...
var begin_qns = {
	...
	stimulus: '<p class="instructions">We will now ask you to complete some questionnaires. '+
						'Please note the options given might differ for different questionnaries.</p>' +
...
var bis = {
	...
	questions: make_qvar(bis_qn,bis_scale),
	preamble: ['<strong>People differ in the ways they act and think in different situations. '+
						 'Read each statement and rate according to the labels. Do not spend too much time on any statment. Answer quickly and honestly.</strong>'],
	horizontal: true,
...
var ocir = {
	type: "survey-multi-choice",
	questions: make_qvar(ocir_qn,ocir_scale),
	horizontal: true,
	preamble: ['<strong>The following statements refer to experiences that many people have in their everyday lives. '+
						 'Rate according to the label that best describes HOW MUCH that experience has DISTRESSED or BOTHERED you during the PAST MONTH.</strong>'],
...
var schizo1 = {
	type: "survey-multi-choice",
	questions: make_qvar(schizo_qn,schizo_scale),
	horizontal: true,
...
var schizo2 = {
	type: "survey-multi-choice",
	questions: make_qvar(schizo_qn2,schizo_scale),
	horizontal: true,
	preamble: ["<strong>Please read the statments and answer whether you agree or disagree with the statements with 'No' or 'Yes' ONLY.</strong>"],
	data: {label: 'schizo', trialType: 'quest'}
};
...
var zungdep = {
	type: "survey-multi-choice",
	questions: make_qvar(zung_qn,zung_scale),
	horizontal: true,
...
// function to make questionnaire variable for LIEBO SOCIAL ANXIETY QUESTIONNAIRE - JL
function make_qvar_lebsocial(qs,scale1,scale2) {
	var q_arr = [];
	for (var i=0; i<qs.length; i++) {
		let scale;
		if (i%2==0){ // all even-numbered indexed questions (i.e. Fear/Anxiety)
				scale = scale1;
		} else { 		 // all odd-numbered indexed questions (i.e. Avoidance)
				scale = scale2;
		}
		q_arr.push({
			prompt: qs[i],
...
var lebsocial1 = {
	type: "survey-multi-choice",
	questions: make_qvar_lebsocial(lebsocial_qn,lebsocial_scale1,lebsocial_scale2),
	horizontal: true,
	preamble: ['<strong>Read each bolded statement carefully and answer two questions about that statement. '+
						 'The first question asks how ANXIOUS or FEARFUL you feel in that situation. The second question asks how often you AVOID that situation. '+
...
var lebsocial2 = {
	type: "survey-multi-choice",
	questions: make_qvar_lebsocial(lebsocial_qn2,lebsocial_scale1,lebsocial_scale2),
	horizontal: true,
	preamble: ['<strong>Read each bolded statement carefully and answer two questions about that statement. '+
						 'The first question asks how ANXIOUS or FEARFUL you feel in that situation. The second question asks how often you AVOID that situation. '+
						 'Please base your ratings on the way that the situations have affected you in the LAST WEEK.</strong>'],
...
var iq_qn2 = ['9) Which figure fits into the missing slot?'+
							'<center><img src="img/ques/mx45_q.png"><br><img src="img/ques/mx45_a.png"><br></center>',
							'10) Which figure fits into the missing slot?'+
							'<center><img src="img/ques/mx46_q.png"><br><img src="img/ques/mx46_a.png"><br></center>',
							'11) Which figure fits into the missing slot?'+
							'<center><img src="img/ques/mx47_q.png"><br><img src="img/ques/mx47_a.png"><br></center>',
							'12) Which figure fits into the missing slot?'+
							'<center><img src="img/ques/mx55_q.png"><br><img src="img/ques/mx55_a.png"><br></center>'];
var iq_qn3 = ['13) All the cubes below have a different image on each side. Select the choice that represents a rotation of the cube labeled X.<br>' +
							'<center><img src="img/ques/rsd3_q.png"></center>',
							...
							'<center><img src="img/ques/rsd4_q.png"></center>',
							...
							'<center><img src="img/ques/rsd6_q.png"></center>',
							...
							'<center><img src="img/ques/rsd8_q.png"></center>'];
var iq_scales = {
	s1: ["2", "3", "4", "5", "6","7"],
...
// function to make questionnaire variable for IQ TEST
function make_qvar_iq(qs,scales,iset) {
	var q_arr = [];
	for (var i=0; i<qs.length; i++) {
	 	let scale;
		if (iset == 1) {
...
var icariq1 = {
	type: "survey-multi-choice",
	questions: make_qvar_iq(iq_qn1,iq_scales,1),
	preamble: ['<strong>Please read each question carefully and answer as best as you can.</strong>'],
	data: {label: 'iq', trialType: 'quest'}
};
var icariq2 = {
	type: "survey-multi-choice",
	questions: make_qvar_iq(iq_qn2,iq_scales,2),
	preamble: ['<strong>Please read each question carefully and answer as best as you can.</strong>'],
	data: {label: 'iq', trialType: 'quest'}
};
var icariq3 = {
	type: "survey-multi-choice",
	questions: make_qvar_iq(iq_qn3,iq_scales,3),
	preamble: ['<strong>Please read each question carefully and answer as best as you can.</strong>'],
	data: {label: 'iq', trialType: 'quest'}
};
...
function make_qvar_alcohol(qs,scales) {
	var q_arr = [];
	for (var i=0; i<qs.length; i++) {
	 	let scale;
		if (i == 0) { // question 1
...
var alcoholadd = {
	type: "survey-multi-choice",
	questions: make_qvar_alcohol(alcohol_qn,alcohol_scales),
	preamble: ['<strong>Now we are going to ask you some questions about your use of alcoholic beverages during the past year. '+
						 'Please be as honest and as accurate as you can be.</strong>'],
...
var apathy = {
	type: "survey-multi-choice",
	questions: make_qvar(apathy_qn,apathy_scale),
...
var eat = {
	type: "survey-multi-choice",
	questions: make_qvar(eat_qn,eat_scale),
...
var genanxiety = {
	type: "survey-multi-choice",
	questions: make_qvar(anxiety_qn,anxiety_scale),
	preamble: ['<strong>Read each statement and select the appropriate response to indicate how you feel right now, that is, at this very moment. '+
						 'There are no right or wrong answers. '+
						 'Do not spend too much time on any one statement but give the answer which seems to describe your present feelings best.</strong>'],
	data: {label: 'anxiety', trialType: 'quest'}
};

// PUSH QUESTIONNAIRE INSTRUCTIONS
qns.push(begin_qns);
// 10 items: bis, ocir, schizo, zungdep, lebsocial, icariq, genanxiety, apathy, eat, alcoholadd
var questlist = [bis, ocir, schizo1, schizo2, zungdep, lebsocial1, lebsocial2, icariq1,
								 icariq2, icariq3, genanxiety, apathy, eat, alcoholadd];
var questlistidx = Array.from(Array(questlist.length).keys());
var shufflequestidxlist  = jsPsych.randomization.shuffle(questlistidx); // 11 items
var shufflequestlist = []; // 10 items (at the end)

// because schizo is split into 2 variables, this loop is necessary to keep them together - JL
// skip on indices 3, 6, 8, 9
const repeatidx = new Set([3,6,8,9]);
shufflequestidxlist.forEach(idx_q => {
	if (repeatidx.has(idx_q)) {
		// do nothing
	} else if (idx_q == 2){
			shufflequestlist.push(questlist[idx_q]);
			shufflequestlist.push(questlist[idx_q+1]);
	} else if (idx_q == 5) {
			shufflequestlist.push(questlist[idx_q]);
			shufflequestlist.push(questlist[idx_q+1]);
	} else if (idx_q == 7) {
			shufflequestlist.push(questlist[idx_q]);
			shufflequestlist.push(questlist[idx_q+1]);
			shufflequestlist.push(questlist[idx_q+2]);
	} else {
			shufflequestlist.push(questlist[idx_q]);
	}
});
